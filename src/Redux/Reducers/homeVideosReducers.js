...
export const channelVideosReducer = (state={
  ...
  const {payload, type} = action
  switch(type){
    case selected_VideoChannel_Request:
    return {
      ...state,
      loading: true
    }
    case selected_VideoChannel_Success:
    return {
      ...state,
      loading: false,
      videos: payload
...
export const SubscribersReducer = (state={
  loading: false,
  channels: []
}, action) =>{
  const {payload, type} = action
